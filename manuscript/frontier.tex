...
\address{
\authorrefmark{1}Faculty of Information Technology \& Computer Science, University of Central Punjab, Lahore, 54000, Pakistan (e-mail: annas.waseem@ucp.edu.pk; ahmadpvt3@gmail.com)}
...
\begin{abstract}
WiFi Channel State Information (CSI) supports device-free human activity recognition (HAR), and a growing body of work moves the inference onto microcontroller-class radios for scale. Almost all of it targets the PSRAM-equipped ESP32-S3, or uses the microcontroller only to collect CSI. The cheapest and most constrained member of the family, the classic ESP32 (ESP32-WROOM-32), has no PSRAM, no vector extensions, and about 150 kilobytes of usable contiguous SRAM, and has not been characterized as an inference target for CSI HAR. We characterize it on the physical device: across two public datasets (UT-HAR and CSI-HAR), with NTU-Fi\_HAR as a third check, we train and quantize a graded set of models, from classical learners through a tiny-CNN sweep to five standard deep architectures, and flash each to the board. An export-pipeline boundary blocks only the recurrent and state-space families through the TensorFlow~2.19 route we use; a Kolmogorov-Arnold model converts but fails to allocate. The convertible feed-forward models all run: a deep CNN and a convolution-augmented Transformer fit within 10 to 54 kilobytes of tensor arena, alongside the tiny CNNs (9 to 68 milliseconds per inference at 240\,MHz). For the architectures we tested, tensor-arena capacity is not the barrier, which corrects a common assumption. The harder challenge is a new person: under subject-independent leave-one-user-out on CSI-HAR the same models fall from 69--96\% to 41--63\%, a mean 29-point drop (an exploratory finding on three users). We also measure per-inference energy with an INA219, compare against the ESP32-S3, and run the full capture-to-label pipeline live for 30 minutes. The contribution is a fair, reproducible deployment map of the most constrained commodity WiFi microcontroller, with all code, models, and firmware released.
\end{abstract}
...
\textbf{This work} & \textbf{classic ESP32} & \textbf{classic ESP32} & \textbf{Y} & \textbf{Y} & \textbf{Y} & \textbf{classical + tiny + deep} & \textbf{int8 + emlearn} & \textbf{N} & \textbf{8--54\,kB arena} & \textbf{9--321\,ms} & \textbf{measured (INA219)} \\
\bottomrule
\end{tabular}}
\end{table*}
...
\emph{Run provenance.} The accuracy numbers in this paper come from several independent training runs, each with its own random seeds: the frontier sweep (Table~\ref{tab:frontier}), the quantization-validation run evaluated on the single exported checkpoint that was actually deployed (Table~\ref{tab:accval}), the per-user and per-class metrics run (Table~\ref{tab:peruser}, Table~\ref{tab:prf}, Figure~\ref{fig:cm}), the deep-tier CSI-HAR run (Table~\ref{tab:deep}), and a companion single-pipeline five-architecture benchmark that supplies the deep UT-HAR accuracies (same training and quantization protocol as Section~\ref{sec:setup}). Because these are distinct seeded runs, the same model can differ by one to two points across tables; this is seed and fold variance, not an inconsistency, and each table names its source notebook in the released artifact.
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\columnwidth]{fig_live_pipeline.pdf}
...
To check that this gap is not an artefact of the three-user CSI-HAR set, we reproduced the same random-versus-subject-independent contrast on two much larger public WiFi-CSI datasets, using a light Random Forest on per-subcarrier amplitude statistics (Table~\ref{tab:gengap-ext}, Figure~\ref{fig:gengap-ext}). The two datasets behave differently, and the difference is itself informative. On CSI-Bench~\cite{csibench2025}, a four-class task on 35 users, accuracy falls from 84.1\% to 51.7\% under a subject-independent split, a graded 32-point drop that remains well above the 25\% chance level, matching the CSI-HAR pattern at larger scale. On MM-Fi~\cite{mmfi2023}, a much harder 27-class daily-activity corpus, this light classifier collapses rather than degrades: subject-independent accuracy falls from 47.0\% to 6.7\% (10 subjects) and from 35.8\% to 4.8\% (all 40 subjects across four environments), essentially the 3.7\% chance level. We therefore report MM-Fi as a chance-level collapse rather than as a graded ``gap'': a drop that lands at chance is a floor effect, and its numerical size is not directly comparable to the above-chance drops, so we do not average it into a single gap figure. The absolute MM-Fi accuracy is low by design, because the classifier is deliberately light (single-frame amplitude statistics on a 27-class problem); the finding is the direction, not a competitive MM-Fi number. Taken together, across datasets spanning 3 to 40 subjects, moving from a subject-overlapping to a subject-independent split removes most or all cross-subject accuracy in every case, so the effect is a property of cross-subject WiFi sensing rather than of the small three-user set, and it does not shrink as the subject pool grows.

\begin{table}[t]
\caption{Cross-subject generalization on WiFi-CSI datasets spanning 3 to 40 subjects. Moving from a random (subject-overlapping) split to a subject-independent split removes most or all cross-subject accuracy in every case. On CSI-HAR and CSI-Bench the subject-independent accuracy stays well above chance, so the drop is a graded gap; on MM-Fi it collapses to the 27-class chance level (3.7\%), a floor effect, so we label those rows ``to chance'' rather than a graded gap because a drop that lands at chance is not comparable in magnitude to the above-chance drops. MM-Fi is reported at 10 subjects (one environment) and at the full 40 (all four environments). The light amplitude-statistics classifier gives modest absolute accuracy by design; the finding is the cross-subject collapse, not a competitive number.}
\label{tab:gengap-ext}
\centering
\footnotesize
\begin{tabular}{lcccc}
\toprule
Dataset (subjects) & Random (\%) & Subject-indep. (\%) & Chance (\%) & Cross-subject drop \\
\midrule
CSI-HAR (3)    & 92.1 & 62.1 & 14.3 & 30.0 pts \\
CSI-Bench (35) & 84.1 & 51.7 & 25.0 & 32.4 pts \\
MM-Fi (10)     & 47.0 & 6.7  & 3.7  & to chance \\
MM-Fi (40)     & 35.8 & 4.8  & 3.7  & to chance \\
\bottomrule
\end{tabular}
\end{table}
