...
Accuracy alone can hide class-dependent behaviour, so for the subject-independent CSI-HAR setting we pool the leave-one-user-out predictions across the three folds, so that every sample is predicted exactly once as an unseen user, and report macro precision, recall, and F1 for a tiny CNN, a Random Forest, and a deep CNN in Table~\ref{tab:prf}. The three models track their accuracy closely, with macro F1 within about a point of accuracy in every case, so no model is inflating its accuracy by collapsing onto a majority class. Figure~\ref{fig:cm} shows the corresponding confusion matrices. The residual errors are physically sensible and shared across models: the dynamic whole-body activities walk and run are occasionally confused with each other, as are the two near-static postures sit down and stand up, whereas fall is separated relatively cleanly. These are exactly the pairs that share a similar amplitude signature on a single link, and they persist across a tiny model and a deep one, which is consistent with the gap being a property of cross-subject CSI rather than of model capacity.
...
\caption{Confusion matrices on CSI-HAR under subject-independent leave-one-user-out (predictions pooled across the three folds) for the tiny CNN, the Random Forest, and the deep CNN. The recurring confusions are the dynamic pair walk and run and the near-static pair sit down and stand up; they share a similar single-link amplitude signature and persist from the tiny model to the deep one.}
\label{fig:cm}
\end{figure*}

We also tested whether the accuracy differences between models are statistically meaningful given only three held-out users. A Friedman test across the tiny CNN, deep CNN, Transformer, and Random Forest over the three folds is not significant ($\chi^2 = 4.44$, $p = 0.22$), and a Wilcoxon signed-rank test between the deep CNN and the Transformer is likewise not significant ($p = 0.25$). With three folds the statistical power is low and we report this honestly, but the finding is consistent with the rest of the paper: on this hardware and these datasets a tiny model is not measurably worse than a deep one, so the case for the small models does not rest on a difference the data cannot support.
...
The two datasets analyzed here are public: UT-HAR \cite{yousefi2017uthar} (obtained through SenseFi \cite{yang2023sensefi}) and CSI-HAR \cite{csihardataset}. The authors collected no new human data. Source code, the quantized and classical models, and the measurement firmware will be made available in a public repository upon publication.
