...
\emph{Hardware constraints.} All device measurements come from a single classic ESP32 board. Energy is measured on that board with an INA219 current sensor (Section~\ref{sec:results-ondevice}); any datasheet-derived figure we quote for context is presented as an estimate over the full 30 to 68\,mA range and not as a single value or bound. We do not run the same models on an ESP32-S3, which has PSRAM and vector (SIMD) extensions; by construction the S3 would fit larger models and run the deep ones faster, so our results characterize the weakest and most constrained commodity target rather than the family's ceiling. For rough context only (not a bound), Hernandez \emph{et~al.}~\cite{hernandez2025tools} report 232\,ms per inference and a 127\,kB footprint (using external PSRAM) for a quantized recurrent CSI model on the ESP32-S3, whereas our feed-forward deep models run in 170 to 321\,ms on the PSRAM-less classic ESP32 through the export pipeline described here. A direct S3 run on the same pipeline is a natural follow-up.
