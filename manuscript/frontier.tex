...
\emph{Hardware constraints.} All device measurements come from a single classic ESP32 board. We report latency and peak memory measured directly on that board, but energy is a datasheet-based simulation rather than a probe measurement, because the board carries no current-sense hardware; a direct study with an INA219 or Joulescope is left to future work. The estimate is nonetheless well grounded: the latency-times-power method is standard \cite{banbury2021micronets}, and our 0.165\,W point is a conservative upper bound relative to the 72.6 to 102.3\,mW that Bolat \emph{et~al.}~\cite{bolat2025ddsnn} measured for int8 CNN inference on an ESP32-S3. We also do not run the same models on an ESP32-S3, which has PSRAM and vector (SIMD) extensions; by construction the S3 would fit larger models and run the deep ones faster, so our results are a lower bound on what the ESP32 family can do, and the point of using the bare classic ESP32 is precisely that it is the weakest and most constrained commodity target. For calibration against the S3, Hernandez \emph{et~al.}~\cite{hernandez2025tools} report 232\,ms per inference and a 127\,kB footprint (using external PSRAM) for a quantized recurrent CSI model on the ESP32-S3; our feed-forward deep models run in 170 to 321\,ms on the weaker PSRAM-less classic ESP32, a comparable latency regime, while their recurrent architecture would not convert to the classic-ESP32 runtime at all (Table~\ref{tab:unsupported}). A direct S3 comparison on the same pipeline is a natural follow-up.
...
C. R. Banbury \emph{et al.}, ``MicroNets: neural network architectures for deploying TinyML applications on commodity microcontrollers,'' in \emph{Proc. Machine Learning and Systems (MLSys)}, vol. 3, 2021, pp. 517--532.

\bibitem{bolat2025ddsnn}
Y. Bolat, I. Murray, Y. Ren, and N. Ferdosian, ``Decentralized distributed sequential neural networks inference on low-power microcontrollers in wireless sensor networks: a predictive maintenance case study,'' \emph{Sensors}, vol. 25, no. 15, p. 4595, 2025, doi: 10.3390/s25154595.
