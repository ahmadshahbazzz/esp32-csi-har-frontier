...
\begin{abstract}
WiFi Channel State Information (CSI) is increasingly used for device-free human activity recognition (HAR), yet most on-device inference relies on microcontrollers with external PSRAM (e.g., ESP32-S3) or offloads computation entirely. This paper characterizes the classic ESP32---the most constrained variant without PSRAM ($\sim$150 KB usable SRAM)---as an on-device inference target. We evaluate a spectrum of models on the UT-HAR and CSI-HAR datasets, spanning classical ML, tiny CNNs, and deep architectures across quantization feasibility, memory footprint, latency, and accuracy. Our results show that memory is not the bottleneck---deep CNNs and Transformers fit easily within a 10--54 KB tensor arena with 9--68 ms latency, whereas recurrent and state-space models fail due to TFLM operator conversion limits. Instead, the primary constraint is cross-subject generalization, where accuracy drops by an average of 29 percentage points (from 69-96\% down to 41-63\%) under Leave-One-User-Out evaluation. This work demonstrates that on-device CSI HAR on commodity microcontrollers is bottlenecked by subject generalization rather than chip memory.
\end{abstract}

\begin{keywords}
Channel state information, human activity recognition, on-device inference, ESP32, cross-subject generalization.
\end{keywords}
...
To support reproducible edge AI research, all code, trained model artifacts, and measurement firmware have been open-sourced. Future work will focus on precise on-device energy profiling using physical current probes, expanding the deployability map to additional microcontroller architectures, and conducting live-capture field validations. Furthermore, we plan to port the failed recurrent, state-space, and Kolmogorov-Arnold models through alternative embedded runtimes---such as microTVM, Apache TVM, and Edge Impulse---to determine whether these deployment failures are intrinsic to the architectures or artifacts of TFLM operator support.
