...
The correction is therefore important: for the feed-forward architectures we tested (a deep CNN and a convolution-augmented Transformer), tensor-arena capacity is \emph{not} the deployment barrier on the classic ESP32; there is no memory wall for these models. The real barriers are the export-pipeline compatibility of the recurrent and state-space families and, for the KAN, an operator-preparation (kernel-compatibility) failure rather than an SRAM ceiling: \texttt{AllocateTensors} invokes each operator's prepare step, and it is the \texttt{FILL} kernel's preparation that aborts, not memory allocation. What makes the tiny and classical tiers the practical choice is not that the deep models cannot fit, but that they are several-fold smaller and faster at comparable accuracy on the saturated benchmark: the 32-channel tiny CNN is about 2.7 times smaller in int8 size and 3.3 times faster than the deep CNN, and the narrower tiny CNNs and the classical learners more so (Section~\ref{sec:results-ondevice}).
...
\begin{table}[t]
\caption{On-device memory and latency with the radio off versus WiFi active (station mode), classic ESP32 at 240\,MHz. The tensor arena is unchanged by the radio; WiFi initialization drops the largest free internal block from 152\,kB to 108\,kB, and during the live run it is about 88 to 92\,kB. The heap left after the arena (right two columns) is the practical headroom for application code and network buffers, which is tight for the deep Transformer. Latency stays within a millisecond of the radio-off value because inference is CPU-bound and the idle driver does not contend for compute.}
\label{tab:wifimem}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
...
To check that this gap is not an artefact of the three-user CSI-HAR set, we reproduced the same random-versus-subject-independent contrast on a much larger public WiFi-CSI corpus, using a light Random Forest on per-subcarrier amplitude statistics (Table~\ref{tab:gengap-ext}, Figure~\ref{fig:gengap-ext}). On CSI-Bench~\cite{csibench2025}, a four-class task on 35 users, accuracy falls from 84.1\% to 51.7\% under a subject-independent split, a graded 32-point drop that remains well above the 25\% chance level, matching the CSI-HAR pattern at more than ten times the subject count. The degradation therefore appears on a many-user dataset and is not specific to the small three-user set. This check uses a light off-device classifier rather than the on-device tiny models and a different task, so it establishes the broader cross-subject phenomenon rather than the on-device deployability result, which rests on CSI-HAR; a fully powered on-hardware cross-subject study on a many-subject, multi-room corpus remains the most important extension.

\begin{table}[t]
\caption{Cross-subject generalization gap on two WiFi-CSI datasets: CSI-HAR (3 users, the on-device dataset) and CSI-Bench (35 users). Moving from a random (subject-overlapping) split to a subject-independent split lowers accuracy by about 30 points in both cases, and in both the subject-independent accuracy stays well above chance, so the effect is a genuine graded gap rather than a floor effect and is not specific to the small three-user set. The CSI-Bench check uses a light off-device Random Forest on amplitude statistics, not the deployed tiny models.}
...
\caption{The cross-subject generalization gap reproduces on a many-user dataset: on both CSI-HAR (3 users) and CSI-Bench (35 users) accuracy drops by about 30 points from a random (subject-overlapping) split to a subject-independent split, staying above chance in both.}
