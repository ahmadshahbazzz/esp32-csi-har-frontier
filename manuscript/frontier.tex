...
Hernandez \emph{et~al.} '25 \cite{hernandez2025tools} & ESP32-S3 & ESP32-S3 & Y & Y & Y & LSTM-DenseNet & int8 & Y & 127\,kB & 232\,ms & n/r \\
...
\begin{table}[t]
\caption{Measured active power and energy per inference on the classic ESP32 (240\,MHz, radio off), captured with the INA219 on the 5\,V board input while a GPIO marker isolates each \texttt{Invoke}. Active power is the mean bus power during inference; energy is active power times measured latency. The measured active current (73 to 77\,mA for the convolutional and Transformer models) exceeds the 68\,mA datasheet figure, so the earlier 50\,mA assumption was not a conservative bound; the statistics MLP draws the least active power (about 300\,mW).}
\label{tab:energy}
\centering
\footnotesize
\begin{tabular}{llccc}
...
\caption{The cross-subject generalization gap reproduces across three datasets spanning 3 to 43 subjects: accuracy drops sharply from a random (subject-overlapping) split to a subject-independent split in every case.}
...
\bibitem{csibench2024}
G. Zhu, Y. He, and co-authors, ``CSI-Bench: A large-scale in-the-wild dataset for multi-task WiFi sensing,'' \emph{arXiv preprint}, 2024. [Online]. Available: \url{https://github.com/guozhenjennzhu/CSI-Bench}
