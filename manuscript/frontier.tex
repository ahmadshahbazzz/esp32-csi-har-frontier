...
% Compact the inter-biography spacing: the class uses "\vskip 4\baselineskip plus 1fil",
% whose stretchy glue spreads short bios across the page. Use a small fixed gap instead.
\renewenvironment{IEEEbiographynophoto}[1]{%
  \if@biographyTOCentrynotmade
    \setcounter{biography}{-1}\refstepcounter{biography}%
    \addcontentsline{toc}{section}{Biographies}\global\@biographyTOCentrynotmadefalse
  \fi
  \refstepcounter{biography}\addcontentsline{toc}{subsection}{#1}%
  \normalfont\biographyfont\interlinepenalty500%
  \vskip 1.5\baselineskip\parskip=0pt\par%
...
\textit{Gap.}~To the best of our knowledge, no prior study characterizes the bare, PSRAM-less classic ESP32 as an inference target for WiFi CSI HAR, nor maps which models across the classical, tiny, and deep spectrum actually fit it. This paper fills that gap.
...
\emph{Deployment comparison.} The comparison above is on accuracy; on the deployment axis the closest prior on-device works run on more capable boards with larger footprints. Armenta-Garcia \emph{et~al.}~\cite{hernandez2025tools} deploy a quantized DenseNet on the PSRAM-equipped ESP32-S3 (92.43\%, 232\,ms, 127\,kB), and Lenka and Chakraborty \cite{lenka2024wisdom} run a compressed int8 DNN on the ESP32-C3 with a RAM budget of a few hundred kilobytes. On the bare PSRAM-less classic ESP32, the weakest member of the family, our deployable models occupy several-fold to an order of magnitude less RAM, at in-distribution accuracy in the same high range: the 16-channel tiny CNN runs in 8 to 13\,kB at 20 to 30\,ms, and even the deep CNN and Transformer allocate within 10 to 54\,kB. These are separate studies on different chips, models, and datasets, so the numbers are contextual rather than a controlled head-to-head (as Table~\ref{tab:priorcompare} notes); the point is that the classic ESP32 hosts the same class of models within a substantially smaller memory budget than the boards used in prior on-device work.

\subsection{The deep tier: which families deploy}
\label{sec:results-walls}
Table~\ref{tab:deep} reports the five deep architectures on both datasets together with the result of flashing every convertible model to the physical device. The outcome is less clear-cut than a blanket ``deep models do not fit.''
...
Table~\ref{tab:ondevice} reports the measured latency and memory for the tiny and classical tiers and for the two deep families that deploy. Every tiny network allocates with a peak tensor arena of only 7.7 to 13.4\,kB, well under both the roughly 150\,kB block the runtime exposes and even the deep CNN's own 10 to 12\,kB. Inference latency ranges from 9\,ms (8-channel tiny CNN on CSI-HAR) to 68\,ms (32-channel on UT-HAR), all far below the roughly 640\,ms needed to accumulate one non-overlapping 64-packet CSI window at the measured packet rate (Section~\ref{sec:results-live}), so inference is never the bottleneck: latency grows with channel width, and the 16-channel tiny CNN, the best accuracy-per-size point, costs 30\,ms on UT-HAR and 20\,ms on CSI-HAR. The flattened MLP has the smallest arena of all despite its large weight file, because its activations are a single flat vector, confirming that the runtime arena and not the model file size is what meets the memory budget. Each latency is a mean over 1000 invocations with a standard deviation below 0.6\,ms, so timing is essentially deterministic within a run (this is within-run variability on one board, not a cross-board interval, which we do not claim from a single device).

The two deep models that deploy run in 226\,ms (CNN, UT-HAR) to 321\,ms (Transformer, CSI-HAR), roughly five to fifteen times slower than the tiny CNNs; on CSI-HAR the Transformer is slower than the deep CNN even though both fit comfortably, consistent with a compute rather than a storage explanation, since self-attention forms sequence-quadratic integer matrix multiplications on a scalar Xtensa core while the convolution reuses a small kernel (the arena agrees: 24 to 54\,kB of attention transients against 10 to 12\,kB). The classical learners are the cheapest model step of all, traversing in 1 to 45\,microseconds with under 1\,kB of working memory; their end-to-end cost is set by the shared five-statistic feature extraction (about 1 to 2\,ms), still roughly an order of magnitude faster than the tiny CNNs. Energy is measured directly with the INA219 (Table~\ref{tab:energy}): gross energy per inference (including the idle baseline) is a few to about twenty-five millijoules for the tiny networks and about 1 to 7\,mJ above idle, and the classical learners draw negligible marginal energy, remaining the cheapest option on either accounting.
...
A fielded sensor must run more than isolated inference, so we ran the entire acquisition-to-prediction pipeline on the classic ESP32 in real time (Figure~\ref{fig:live}). The board associates to a 2.4\,GHz access point as a station and pings the gateway so that received packets arrive steadily; the esp-wifi CSI callback delivers per-packet subcarrier data, from which the device forms $T{=}64$ by 52-subcarrier amplitude windows, applies the same per-window normalization as training, int8-quantizes to the deployed input scale, and runs the tiny CNN under TensorFlow Lite Micro. We ran this continuously for 30 minutes; Table~\ref{tab:live} summarizes the result. The pipeline sustained a CSI arrival rate of about 100 packets per second and classified 2{,}825 windows with no dropped windows and no overflow, at a steady 37.6\,ms post-window latency (from the last packet of a window to the label; the capture-to-label time is dominated by the roughly 640\,ms needed to accumulate a 64-packet window at about 100 per second) and an acquisition-bound rate of 1.57 windows per second, while the free internal SRAM varied by only about 2.5\,kB over the whole run and the device never crashed or reset. The full sensing loop, from capture to label, therefore runs and stays stable on the bare PSRAM-less device for the whole run, and not just the isolated inference stage measured elsewhere in this paper.

We also measured live in-domain recognition accuracy on this board. Because a model trained on a public corpus is out of domain for this radio and room, we collected a small labelled set on the device itself: one person performed the seven CSI-HAR activities in front of the board across two sessions, with the position and access point changed between them, and every window was captured through the same on-device windowing, normalization, and int8 path used for deployment (881 and 805 windows, 1{,}686 in total). We then trained the 16-channel tiny CNN on this on-device data (Table~\ref{tab:liveacc}). Within a single session the model reaches 46.9\% over the seven activities (chance 14.3\%), but under a leave-one-session-out split, training on one session and testing on the other, accuracy falls to 25.8\%. We did not obtain synchronized ground-truth labels during the continuous 30-minute run, so this is a separate on-device live-data experiment, a proof of operation rather than a performance benchmark. The absolute numbers are modest, as expected from a single low-cost antenna and a small self-collected set, but the trend is what matters: recognition of real over-the-air CSI works well above chance, yet accuracy drops by about twenty points from within-session to a different session of the same person. This session-to-session collapse is a within-subject domain shift rather than a cross-subject test, since it uses one person across two sessions, but it points the same way as the cross-subject generalization gap of Section~\ref{sec:results-gap}: on live on-device data, as on the public datasets, generalization to unseen conditions rather than deployment is the practical limiting factor on the classic ESP32.

\begin{table}[t]
\caption{Live deployment on the classic ESP32: the full capture-to-label pipeline run continuously for 30 minutes (2.4\,GHz station, CSI from received packets, 16-channel tiny CNN). ``Post-window latency'' is the time from the last packet of a completed 64-packet window to the emitted label (preprocessing plus inference); it is not the capture-to-label time, which is dominated by the roughly 640\,ms needed to accumulate 64 packets at about 100 per second. The live window rate (1.57/s) is therefore bounded by non-overlapping window acquisition, not by inference, whose isolated rate is far higher (Section~\ref{sec:discussion}). ``Packet / window loss'' is measured at the CSI callback (buffer level): no delivered callback was dropped and no window overflowed in this run; upstream 802.11 losses before the callback are not observable here. Live in-domain classification accuracy is measured separately from an on-device labelled set (Table~\ref{tab:liveacc}).}
\label{tab:live}
\centering
\footnotesize
\begin{tabular}{lr}
...
\begin{table}[t]
\caption{Live in-domain recognition accuracy on the classic ESP32, from real over-the-air CSI captured and windowed on the device (seven CSI-HAR activities, one person, two sessions of 881 and 805 windows recorded with the setup changed between them; deployed 16-channel tiny CNN). Within-session accuracy uses an 80/20 split of one session and is optimistic because the on-device windows overlap; the leave-one-session-out figure (train one session, test the other, mean of both directions over three seeds) is the clean cross-condition number. Chance is 14.3\%.}
\label{tab:liveacc}
\centering
\footnotesize
\begin{tabular}{lc}
...
Evaluation & Accuracy (\%) \\
\midrule
Within-session (80/20, optimistic)   & 46.9 \\
Cross-session (leave-one-session-out) & 25.8 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[tbp]
\centering
\includegraphics[width=\textwidth]{fig_live_pipeline.pdf}
...
\caption{The cross-subject generalization gap on CSI-HAR. The same models score much higher on a random split (light bars) than under subject-independent leave-one-user-out (dark bars); the number above each pair is the accuracy lost per model. The mean drop of 29 points is the cost of recognizing an unseen person, and it is invisible to a random split.}
\label{fig:gengap}
\end{figure}

To check that this gap is not an artefact of the three-user CSI-HAR set, we reproduced the same random-versus-subject-independent contrast on a much larger public WiFi-CSI corpus, CSI-Bench~\cite{csibench2025}, a four-class task on 35 users, with a subject-disjoint train/test partition and the same 16-channel tiny-CNN architecture as the live deployment (Table~\ref{tab:gengap-ext}, Figure~\ref{fig:gengap-ext}). We ran two classifiers there: the 16-channel tiny CNN and, as a light off-device reference, a Random Forest on per-subcarrier amplitude statistics. The 16-channel tiny CNN falls from 93.3\% on a random split to 62.6\% under a subject-independent split, a 30.8-point drop, and the Random Forest from 84.1\% to 51.7\% (32.4 points); both remain well above the 25\% chance level. The gap therefore holds on a many-user dataset for the deployed architecture itself, and not merely for a proxy classifier, and is not specific to the small three-user set. It is measured off-device on a different four-class task, so it establishes the breadth of the cross-subject phenomenon rather than the on-device deployability result, which rests on CSI-HAR; a fully powered on-hardware cross-subject study on a many-subject, multi-room corpus remains the most important extension.

\begin{table}[t]
\caption{Cross-subject generalization gap across datasets and classifiers. Moving from a random (subject-overlapping) split to a subject-independent split lowers accuracy by about 30 points in every case, and the subject-independent accuracy stays well above chance throughout, so the effect is a genuine graded gap rather than a floor effect and is not specific to the small three-user set. On the 35-user CSI-Bench corpus the gap holds for the 16-channel tiny CNN (30.8 points) as well as the light off-device Random Forest reference (32.4 points).}
...
More runs on the classic ESP32 than its reputation suggests. On the PSRAM-less classic ESP32 the deployable options for WiFi CSI HAR include the classical learners, the tiny CNNs, and a deep CNN and convolution-augmented Transformer, all of which allocate and run within tens of kilobytes. The small models are the efficient choice, not the only one that fits: on the saturated UT-HAR benchmark a statistics MLP reaches 95.3\% and the 32-channel tiny CNN 96.7\% (a 37.6\,kB int8 model), within a point or two of the deep models at a fraction of the size and latency, so a practitioner should start with the tiny and classical tiers and reach for a deep CNN or Transformer only if accuracy demands it. The barriers that remain are narrower than a blanket memory limit. Convertibility is the hard gate: recurrent and state-space models do not reach the runtime at all without custom operators, because their fused recurrent and dynamic-tensor operators are outside the builtin set; the Chebyshev-KAN converts but fails to allocate as exported by the default converter, an operator-mapping limitation rather than a memory ceiling, since the much larger CNN and Transformer allocate comfortably on the same device. We initially assumed a memory wall blocked all deep models, and flashing them to the board showed that assumption was wrong, which is the value of bare-device characterization over a GPU-only or PSRAM-equipped comparison, and it also explains why prior on-device CSI work, navigating the same conversion limits, tends to favour convolutional models.

\subsection{Generalization is the binding constraint}
The two halves of the study point in the same direction: once the deep CNN and Transformer are shown to run on the bare board, compute and memory no longer limit CSI HAR on this hardware, while the leave-one-user-out results show a mean 29-point drop when the test person is unseen (Section~\ref{sec:results-gap}). A plausible explanation, consistent with this limited evidence, is physical rather than statistical: CSI amplitude encodes the multipath channel, shaped by the person's body geometry, gait, and position relative to the link, so an unseen person is an out-of-distribution input whose channel signature the model has not learned. This is why increasing capacity did not help (the larger Transformer was no more robust across people than the tiny CNN) while a short per-user calibration did (Figure~\ref{fig:personalization}). Effort is therefore better spent on generalization than on shrinking the model, and subject-independent accuracy should be reported as standard practice, since a random split on a small multi-user corpus reports a number the deployed device will not reach. Cross-subject methods such as domain adaptation are the substantive open problem. This also reinforces why the smallest models suit the most constrained hardware: the classical learners are orders of magnitude smaller, run in microseconds atop a shared 1 to 2\,ms feature-extraction step, and stay competitive on the saturated benchmark, so where a sensing task must share a microcontroller their predictability and small footprint are decisive.

\subsection{Practical deployment under a live radio}
A fielded sensor runs a four-stage real-time pipeline (Figure~\ref{fig:pipeline}), which we built and ran live on the classic ESP32 (Section~\ref{sec:results-live}). Two practical considerations follow. First, timing is comfortable: a tiny CNN classifies a window in tens of milliseconds, well above the rate at which non-overlapping activity windows arrive, and even the deep models remain adequate for activity-scale events. Second, sensing and inference must share the chip. The ESP32 is dual-core, so acquisition and inference run on separate cores, but an active WiFi stack claims internal SRAM, leaving about 88 to 92\,kB free in the live run; every deployable model still fits and runs there with latencies within a millisecond of the radio-off figures (Supplementary Material, Table~S7). System-level headroom, not the arena, is then the real constraint: a 54\,kB Transformer arena leaves little room for a concurrent networking application, whereas the tiny and classical tiers leave ample margin. Runtime stability follows from the static tensor arena, which performs no per-inference allocation and so cannot leak or fragment; over the 30-minute run the latency did not drift and no window was lost, though multi-hour operation and thermal drift remain untested.

\subsection{Limitations}
We group the limitations into three kinds so that the scope of the claims is unambiguous.

\emph{Dataset size and diversity.} The generalization results rest on CSI-HAR, which has only three users, so the leave-one-user-out evaluation runs over three folds and the cross-subject numbers, while consistent, are a demonstration of the effect rather than a large-scale study; the three-user size is also why we treat the significance tests as exploratory (Section~\ref{sec:results-perclass}). UT-HAR is larger but saturated and not subject-partitioned, so it cannot show the gap. We add NTU-Fi\_HAR as a third dataset for the frontier (Section~\ref{sec:results-frontier}), which confirms the accuracy-versus-cost ordering but is itself saturated and single-session. To test the cross-subject gap on more than three users we reproduced it on a larger public corpus, CSI-Bench (35 users), where both the 16-channel tiny CNN (30.8 points) and a light Random Forest reference (32.4 points) show the same graded drop that stays above chance (Section~\ref{sec:results-gap}, Table~\ref{tab:gengap-ext}). That check is off-device and on a different four-class task, so a fully powered on-hardware cross-subject study on a many-subject, multi-room corpus remains the most important extension.
...
\section{Conclusion}
We measured a deployability frontier for WiFi CSI-based human activity recognition on the most resource-constrained commodity WiFi microcontroller, the classic PSRAM-less ESP32. By flashing each model to the physical board at 240 MHz, we found that deep feed-forward architectures, including a full deep CNN and a convolution-augmented Transformer, run on the bare chip within a 10 to 54 KB tensor arena at 170 to 321 ms per inference, while tiny CNNs run in 9 to 68 ms using an 8 to 13 KB arena. On-device feed-forward CSI inference is therefore not blocked by a chip-level memory wall, which corrects a common assumption.
Instead, deployment friction stems from two distinct factors. On the software side, recurrent, state-space, and Kolmogorov-Arnold architectures fail due to runtime conversion and memory allocation limits within TensorFlow Lite for Microcontrollers (TFLM). On the algorithmic side, subject-independent leave-one-user-out evaluation reduces accuracy sharply, by an average of 29 percentage points (from 96\% to 63\% for the best model) on unseen individuals. Across the datasets evaluated here, subject-independent generalization is therefore a more critical bottleneck than on-chip memory for CSI HAR on this hardware; a fully powered many-subject on-device study remains the key step toward a broader real-world deployment claim.
In addition to the frontier, we characterized the classic ESP32 on real hardware: per-inference energy measured with an INA219 current sensor, a comparison against the PSRAM-equipped ESP32-S3, and a 30-minute live capture-to-label pipeline that sustained real WiFi traffic with no window loss or memory drift. All code, trained models, and measurement firmware are released. Future work includes higher-bandwidth energy profiling and multi-board characterization, extending the deployability map to additional microcontroller architectures, and live-capture field validation that reports in-domain recognition accuracy. We also plan to port the recurrent, state-space, and Kolmogorov-Arnold models through alternative embedded runtimes such as microTVM, Apache TVM, and Edge Impulse, to test whether these failures are intrinsic to the architectures or artifacts of TFLM operator support.
